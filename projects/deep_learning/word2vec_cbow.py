...
# define wether we use CBOW or skip-gram
model_type = 'cbow'
data_preloaded = True
...
if data_preloaded:  # all necessary data is proccessed in save in pickles.
...
else:  # first run when datasets need to be generated.
...
# Step 3: functions to generate batches for different algorithms
# Step 3.1 generate a training batch for the skip-gram model.
...
# Step 3.2 generate a training batch for the continious bag of words model (aka CBOW).
def generate_batch_cbow(data, batch_size, num_skips, skip_window):
    ...
    for i in range(batch_size):
        mask = [1] * span
        mask[skip_window] = 0
        batch[i, :] = list(np.compress(mask, buffer))  # all surrounding words. VD: swap the arguments
...
if model_type == 'cbow':
    batch, labels = generate_batch_cbow(data, batch_size=8, num_skips=2, skip_window=1)
...
with tf.Session(graph=graph) as session:
    ...
    for step in xrange(num_steps):
        ...
        if model_type == 'cbow':
            batch_inputs, batch_labels = generate_batch_cbow(data, batch_size=batch_size, num_skips=num_skips,
                                                skip_window=skip_window)
        elif model_type == 'skip-gram':
            batch_inputs, batch_labels = generate_skipgram_batch(batch_size=batch_size, num_skips=num_skips, skip_window=skip_window)
        else:
            raise Exception("incorrect model type")
